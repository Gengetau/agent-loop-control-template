...
`reports/` stores execution evidence. Codex reports should explain what was inspected, what changed, how validation was performed, and what uncertainty remains.
...
`signals/` stores machine-readable events. Signals let n8n or another monitor discover that a loop is waiting for review or that a review verdict is ready.
